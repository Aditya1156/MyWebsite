...
${originalMessage}
  ...
  return (
    <AnimatedSection id="hire-me" className="py-24 bg-charcoal text-cream">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <motion.button 
              ...
              className="w-full bg-orange text-cream font-bold py-4 px-8 rounded-lg transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-orange-light focus:ring-offset-2 focus:ring-offset-charcoal disabled:bg-orange/50 disabled:cursor-not-allowed"
            ...
            {status === 'success' && <p className="text-center text-green-400">✅ Message sent successfully! I'll be in touch soon.</p>}
...
const InputField: React.FC<{ label: string, name: string, type: string, placeholder: string }> = ({ label, name, type, placeholder }) => {
  ...
  return (
    <motion.div variants={fieldVariants} className="transition-transform duration-300 ease-out focus-within:-translate-y-1">
      <label htmlFor={name} className="block text-sm font-medium text-cream/80 mb-2">{label}</label>
      <motion.input 
        type={type} 
        name={name} 
        id={name} 
        ...
        required
        className="w-full bg-charcoal/50 border border-cream/20 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-orange transition-all"
  ...
  return (
    <motion.div variants={fieldVariants} className="transition-transform duration-300 ease-out focus-within:-translate-y-1">
      ...
      <motion.select
        ...
        id={name}
        required
        className="w-full bg-charcoal/50 border border-cream/20 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-orange transition-all appearance-none"
  ...
  return (
    <motion.div variants={fieldVariants} className="transition-transform duration-300 ease-out focus-within:-translate-y-1">
      ...
      <motion.textarea
        ...
        rows={4}
        required
        className="w-full bg-charcoal/50 border border-cream/20 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-orange transition-all resize-none"
        onFocus={() => setIsFocused(true)}
        onBlur={() => setIsFocused(false)}
        animate={{ boxShadow: isFocused ? "0 0 0 4px rgba(255, 107, 0, 0.2)" : "0 0 0 0px rgba(255, 107, 0, 0)" }}
        transition={{ type: 'spring', stiffness: 500, damping: 30 }}
