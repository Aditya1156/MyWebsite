...
import { GoArrowUpRightIcon } from './icons';
...
const CardNav: React.FC<CardNavProps> = ({
  ...
  useEffect(() => {
    const totalElements = (items?.length || 0) + 1; // +1 for mobile CTA
    cardsRef.current = cardsRef.current.slice(0, totalElements);
  }, [items]);
  ...
  const createTimeline = () => {
    ...
    const elementsToAnimate = cardsRef.current.filter(Boolean) as HTMLElement[];
    ...
    tl.to(elementsToAnimate, { y: 0, opacity: 1, duration: 0.4, ease, stagger: 0.08 }, '-=0.1');
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-[1.1rem] z-[2]">
          ...
          <div className="logo-container flex items-center md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 order-1 md:order-none">
            <a href="#hero" aria-label="Go to homepage" onClick={(e) => handleLinkClick(e, '#hero')}>
                {typeof logo === 'string' && logo.endsWith('.svg') ? (
                    <img src={logo} alt={logoAlt} className="logo h-[35px]" />
                ) : (
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            ...
            >
              <div className="nav-card-label font-display tracking-[-0.5px] text-[18px] md:text-[22px] font-bold">
                {item.label}
              </div>
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => (
                  ...
                  >
                    <GoArrowUpRightIcon className="nav-card-link-icon shrink-0" aria-hidden="true" />
                    {lnk.label}
                  </a>
                ))}
              </div>
          ...
          <div
            ...
            className="nav-card select-none relative rounded-[calc(0.75rem-0.2rem)] min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:hidden"
            style={{ backgroundColor: buttonBgColor }}
          >
             <a
                href="#hire-me"
                className="w-full h-full flex items-center justify-center text-center font-display tracking-[-0.5px] text-[18px] font-bold no-underline"
